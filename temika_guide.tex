...
If you try to start up Temika and get an error saying \verb|Address already in use| (see image below), then someone else is still logged in on the PC and has not closed the Temika software. You can see who else is still logged in by typing \verb|finger| in the terminal. Then either ask that person to logout or press \verb|Ctrl| \verb|Alt| \verb|Fx| where x can be any of the numbers 1-12; eventually you should find the other user logged in then you can close temika. Repeat to find your own login screen again. Note: this may become redundant if Jurij ends the possibility for multi-login.
